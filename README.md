...
#7.write a query to print the number of orders placed by the customers after they becaming a member.(given by Sir)
...
with CTE1 AS (
select s.*,product_name,mb.join_date  from sales as s inner join menu as m using(product_id)
